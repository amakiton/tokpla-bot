...
| `tokpla_boss_round_trips` | 🔁 v6.337 จำนวนทริปล่าบอส "ของรอบนี้" `{k:คีย์รอบ, n}` — เพดาน 2 ทริป/รอบ |
| `tokpla_boss_round_done` | 🔒 v6.344 รอบที่จบแล้ว `{k:คีย์รอบ, why, at}` — ตั้งเมื่อ ฆ่าได้/บอสหาย/ประตูบอกว่ารอบปิด · บล็อกการออกล่าซ้ำ + ปลดล็อกระบบที่ป้าย "รอบเปิด" เคยแช่ไว้ (หยุดตกปลา/หีบ/หนีถ้ำ) · คีย์รอบเปลี่ยน = ปลดเอง |
| `tokpla_boss_rota` | 🎯 v6.355 "รอบเวลาไหนเจอบอสตัวไหน" `{"1170":{"ชื่อบอส":n}, _seeded:1}` — ใช้ทำนายเพื่อติดเหยื่อจุดอ่อนล่วงหน้า · seed ครั้งเดียวจากสถิติเดิม |
...
| `tokpla_boss_anchor` | ⏱️ v6.362 เวลาบอสเกิดแบบสัมบูรณ์ `{at, readAt, secLeft, why}` — แปลงจากตัวนับบนจอทันทีที่อ่านได้ (แม่นระดับวินาที) · หมดอายุเมื่ออ่านไว้เกิน 6 ชม. หรือเลยเวลาไป 15 นาที |
| `tokpla_boss_slots` | ⏱️ v6.362 ตารางที่ "วัดจากของจริง" `{นาทีของวัน(ปัดลง10): {n, first, last, mins[]}}` — จดตอนเห็นบอสในฉากจริง · เจอ ≥ 2 ครั้ง = รวมเข้าตารางเอง (เกมเปลี่ยนเวลาก็ตามทัน) |
| `tokpla_boss_accuracy` | ⏱️ v6.362 คู่ (เวลาจริง, คลาดกี่วินาที, แหล่งที่ทำนาย) 60 รอบล่าสุด — ตัวพิสูจน์ว่านาฬิกาบอสแม่นจริงไหม (ดู `/bosstime`) |
| `tokpla_daily` | 📅 v6.378 บัญชีรายวัน `{"YYYY-MM-DD": {revenue,baitCost,casts,catches,coffeeCost,potionCost,bossFights,bossKills,shards,firstAt,lastAt}}` — เก็บ 3 วันล่าสุด · **แยกจาก tokpla_bot_profit ทั้งหมด** · ดูที่ `/today` |
| `tokpla_xp_hist` | 🎓 v6.371 ประวัติแถบ XP `[{t,lv,cur,max}]` (ring 200) — จดทุก 3 นาทีระหว่างฟาร์ม · ใช้วัด XP/ชม. จริง + ETA เลเวลถัดไป (`/xp`) |
| `tokpla_hit_gap` | 🔍 v6.360 ช่องว่างยาวระหว่างการตีบอส `[{t,gap,m,b,w:["สาเหตุ:ms",…],f:พื้นของโหมด}]` — ตอบว่าเวลา 57% ที่หายไปอยู่กับอะไร (ดู `/gap`) · เกณฑ์แยกตามโหมด: melee/cast พื้น 900ms · charge พื้น 2,250ms |
| `tokpla_charge_ab` | ⚡ v6.329 A/B โหมด charge — ⚠️ v6.360 พบว่า**เก็บได้ 0 บล็อกจาก 4 ไฟต์** (ตัวกรอง `gap > act/2` ตัดหมด) · ทิ้งติดกัน 8 บล็อก = หยุดสลับ ใช้ชาร์จเต็ม |
