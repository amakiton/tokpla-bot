...
(function () {
  ...
  let bagHeavySaid = false, bagHeavySellAt = 0;   // ⚖️ v6.348: เตือน "หนักเกินพิกัด" ครั้งเดียวต่อรอบ · จังหวะขายเชิงรุกตอน 80%
...
`
  ...
  // ล็อกดิบ — "รอบนี้จบแล้วหรือยัง" ตามที่บันทึกไว้จริง ไม่สนใจว่ารอบถัดไปใกล้แค่ไหน
  //   ⚠️ ใช้ตัวนี้เมื่อคำถามคือ **"ยังมีบอสให้ตีในถ้ำตอนนี้ไหม"** (เช่นการตัดสินใจเดินผ่านประตู)
  function bossRoundDoneRaw() {
    try {
  ...
  // ⏳ v6.357 — **ล็อกต้องหมดอายุก่อนถึงรอบถัดไป ไม่ใช่ตอนถึงเป๊ะ ๆ**
  //   ปมที่เจอสด 1/8: คีย์รอบเปลี่ยนตอน 20:00 พอดี → ก่อนหน้านั้นล็อกของรอบ 19:30 ยังคาอยู่
  ...
  //   (หน้าต่างเปิดยาว 60 นาที ประตูจึงยังเปิดอยู่) ⇒ เข้าไปก็ได้ถ้ำเก่าที่บอสตายไปแล้ว = ห้องว่าง
  //   คืนจำนวน ms ที่ต้อง "ยืนรอหน้าประตู" · 0 = เข้าได้เลย
  function bossEntryHoldMs() {
    try {
      const b = bossRoundBounds();
      if (!b.next) return 0;
      const left = b.next - Date.now();
      if (left <= 0) return 0;                                   // ถึงเวลาแล้ว เข้าได้
      // ใช้ล็อก **ดิบ**: ถ้ารอบปัจจุบันยังไม่จบ (บอสอาจยังมีเลือด) การเข้าไปตอนนี้ถูกต้อง ไม่ต้องรอ
      if (!bossRoundDoneRaw()) return 0;
      // รอบปัจจุบันจบแล้ว + หน้าต่างเก่ายังเปิดอยู่ = ประตูจะพาเข้าถ้ำเก่าที่ว่างเปล่า → ต้องรอ
      const sincePrev = b.prev ? Date.now() - b.prev : Infinity;
      if (sincePrev > SERVER_RAID_WINDOW_MIN * 60000) return 0;   // หน้าต่างเก่าปิดแล้ว → ประตูล็อกเอง ปล่อยระบบเดิมจัดการ
      return Math.min(left, 20 * 60000);                          // เพดานกันรอค้าง (ตารางจริงห่างกันไม่เกิน 30 นาที)
    } catch { return 0; }
  }
  ...
  async function bossTravelTo(targetMap) {
    ...
    //   พอถึงเวลาเกิดรอบใหม่ ทริปถัดไปก็กลับไปใช้ A* ตามปกติ
    if (gameWalker() && !(isBossMap(targetMap) && bossEntryHoldMs() > 0)) {
    ...
    let lastGateWaitSay = NEVER;                    // 🚪 v6.334: throttle ข้อความ "ประตูยังไม่เปิด — รอเงียบๆ"
    for (let hop = 0; hop < 10 && enabled && (isOn('bossHunt') || mythicActive()); hop++) {   // v6.132: ล่าปลาเทพใช้ bossTravelTo ด้วย — เดิม bossHunt ปิด = เดินไม่ออกเงียบๆ
      ...
      // 🚪 v6.379 — **เดินไป "กลางโซนประตู" ไม่ใช่มุมบนซ้าย** (ค่าจริงจาก /api/config → mapZones)
      //   village→boss_cave = x24 y404 กว้าง 44 สูง 240 ⇒ มุมกับกลางโซนห่างกันถึง **122px**
      //   มุมมักชิดขอบแมพ (x24 = เกือบติดขอบซ้าย) และทำให้ทุกตัวที่วัดจาก "กลางโซน" ตัดสินผิดตามไปหมด
      //   (ดูบั๊ก _atGate ใน bossWaitGate — รอบ 22:30 2/8 เสียทั้งรอบเพราะเรื่องนี้)
      const exCx = exit.x + (exit.width || 0) / 2, exCy = exit.y + (exit.height || 0) / 2;
      const r = await bossWalkTo(exCx, exCy, { thresh: 24 });
      ...
      await bossWalkTo(exCx, exCy + (exit.height ? 20 : 0), { thresh: 10, maxMs: 4000 });
      ...
      if (gateToCave) {
        // ⏳ v6.357: ยืนรอหน้าประตูจนถึงเวลาเกิดของรอบใหม่ — เข้าก่อนเวลา = ได้ถ้ำของรอบเก่า (ห้องว่าง)
        //   รอแบบคน (bossGateIdle) ไม่ดันประตู · เช็คใหม่ทุกครั้ง เผื่อสถานการณ์เปลี่ยนกลางคัน
        {
          let holdSaid = false, holdT0 = now();
          while (enabled && (isOn('bossHunt') || mythicActive()) && bossEntryHoldMs() > 0 && now() - holdT0 < 20 * 60000) {
            if (bossAbortTravel) break;
            if (!holdSaid) {
              holdSaid = true;
              const secs = Math.round(bossEntryHoldMs() / 1000);
              say(`⏳ ถึงปากถ้ำแล้ว แต่รอบใหม่ยังไม่เริ่ม — ยืนรออีก ~${secs} วิ (เข้าตอนนี้จะได้ถ้ำของรอบเก่าที่บอสตายแล้ว)`);
              bossEvent(`⏳ ยืนรอหน้าประตู ${secs} วิ ให้ถึงเวลาเกิดรอบใหม่ก่อน — ประตูพาเข้า "ถ้ำที่ active ตอนนั้น" (หลักฐานสด 1/8: เข้า 19:59 ได้ naga_vortex ว่าง · 20:00 ถึงเป็น boss_cave ที่มีบอส)`);
            }
            await bossGateIdle();
          }
          if (holdSaid) bossEvent('⏳ ถึงเวลาเกิดรอบใหม่แล้ว — เข้าถ้ำได้');
        }
      ...
      if (bossMapId() === cur) {
        ...
        //   รีทรายทุก 800 มิลลิวินาที = เดินชนประตูรัวๆ (ต้นเหตุที่ 2 ของอาการในวิดีโอ) → รอเงียบๆ ตามตัวนับแทน
        const _gs = bossGateState();
        if (_gs.locked) {
          mapChangeFails = 0;                                   // ประตูล็อกไม่นับเป็น "ตัวติด" (กันเรียกปุ่มกู้แมพผิดจังหวะ)
          if (now() - lastGateWaitSay > 30000) { lastGateWaitSay = now(); say(`🚪 ประตูยังไม่เปิด${_gs.sec != null ? ` (อีก ${_gs.sec} วิ)` : ''} — รอเงียบๆ ไม่ดันประตู`); }
          await sleep(_gs.sec != null && _gs.sec > 5 ? 2500 : 900);
          continue;
        }
  ...
  // 🚪 v6.334 — **สถานะประตูถ้ำ "จากที่เกมประกาศเอง"** (ผู้ใช้ส่งวิดีโอ 10:29: บอทเดินชนกำแพงรัวๆ คนอื่นเห็นว่าเป็นบอท)
  //   หลักฐานจาก bossGateProbe ที่เก็บไว้ตั้งแต่ v6.242 **แต่ไม่เคยเอามาใช้ตัดสินใจเลย — แค่จดไว้ดู**:
  //     10:27:37 "ประตูบอสยังไม่เปิด | รอบต่อไปเปิดในอีก | 2 นาที 22 วิ" … 10:29:55 "0 นาที 05 วิ"
  //   คือเกม **บอกวินาทีที่ประตูจะเปิดเป๊ะๆ** อยู่บนจอตลอด → ไม่ต้องเดา ไม่ต้อง "ลองดัน" สักครั้ง
  //   ของเดิมทำตรงข้าม: ก้าวถอย→ก้าวเข้าทุก 3.5 วิ ตลอด 2.5 นาที ≈ **40 ครั้ง** = ลายเซ็นบอทเต็มๆ
  //   ⚠️ นับถอยหลัง = ตัว "พิสูจน์ความสด" ในตัว (เลขลดทุกวินาที) → เชื่อได้เต็มที่ · มีแต่ป้ายไม่มีตัวนับ = เชื่อแต่แตะเช็คทุก 45 วิ
  const GATE_LOCK_RE = /ประตูบอสยังไม่เปิด|Boss gate is not open|gate is locked/i;
  let gateStCache = null, gateStAt = NEVER;
  function bossGateState() {
    if (gateStCache && now() - gateStAt < 700) return gateStCache;   // สแกน DOM แพง — cache สั้นๆ (ลูปเรียกถี่)
    gateStAt = now();
    let locked = false, sec = null;
    try {
      // ⚠️ ป้าย/ตัวเลขอยู่คนละ element ("ประตูบอสยังไม่เปิด" · "รอบต่อไปเปิดในอีก" · "2 นาที 22 วิ" — เห็นจาก probe จริง)
      //   ห้ามใช้ gameTextMatch ตรงๆ: มันคืน **ตัวแรกที่ match** ซึ่งอาจเป็นก้อน "รอบต่อไปเปิดในอีก" ที่ไม่มีตัวเลข
      //   (กลุ่มใน regex เป็น optional ทั้งคู่ → match ได้แต่ไม่มีค่า) → สแกนทั้งหน้าแล้วเก็บอันที่ "มีตัวเลขจริง"
      for (const e of document.querySelectorAll('h1,h2,h3,h4,div,span,p')) {
        if (isBotUI(e) || e.offsetParent === null || e.children.length > 3) continue;
  ...
  async function bossGateIdle() {
    if (!gateIdleNextAt) gateIdleNextAt = now() + randInt(25000, 70000);
    if (now() < gateIdleNextAt) { await sleep(randInt(600, 1100)); return; }
    gateIdleNextAt = now() + randInt(25000, 70000);
    const dirs = ['left', 'right', 'up', 'down'];
    const d = dirs[randInt(0, 3)];
    bossHold(d); await sleep(randInt(120, 260)); bossReleaseAll();
    if (randInt(0, 1)) {                                   // ครึ่งหนึ่งของครั้ง เดินกลับที่เดิม (เหมือนคนขยับเท้า)
      await sleep(randInt(250, 600));
      bossHold({ left: 'right', right: 'left', up: 'down', down: 'up' }[d]);
  ...
  async function bossWaitGate(cur, toMap, exitZone) {
    ...
    let lastFarSay = NEVER;                     // v6.379: throttle log "ยังไม่อยู่ในโซนประตู" (สาขาที่เคยเงียบสนิท)
    let gatePushes = 0;                         // v6.382: ดันประตูไปกี่ครั้งแล้วหลังประตูเปิด (กันเดินชนกำแพงรัว)
    ...
    while (enabled && (isOn('bossHunt') || mythicActive()) && now() - t0 < maxMs) {
      ...
      if (!said) {
        ...
        say(`🚪 ถึงปากถ้ำบอสแล้ว — ${_g0.sec != null ? `เกมบอกว่าประตูเปิดอีก ${_g0.sec} วิ · ยืนรอเฉยๆ แล้วเข้าทันทีที่เปิด` : 'ประตูล็อกจนถึงเวลา · จะพุ่งเข้าทันทีที่เปิด'} (บอสอยู่แค่ 1-3 นาที)`);
        bossEvent(`🚪 รอหน้าประตูถ้ำ${_g0.sec != null ? ` (อ่านตัวนับได้: อีก ${_g0.sec} วิ)` : ''} — เข้าถ้ำไม่ได้ก่อนเวลา`);
      }
      ...
      // 🔴 v6.379 — **ต้นตอที่ทำให้เสียรอบบอสซ้ำ ๆ: โซนประตูเป็นสี่เหลี่ยม แต่วัดด้วยรัศมีวงกลม 80px**
      //   ค่าจริงจากเกม (/api/config → mapZones.village.exits): boss_cave = x24 y404 **กว้าง 44 สูง 240**
      //   ⇒ ครึ่งเส้นทแยง = 122px · ตัวยืนที่ (24,434) ซึ่ง **อยู่ในโซนเต็มตัว** ก็ยังห่างจากกลางโซน 93px
      //   ⇒ `_atGate` เป็น **เท็จตลอดกาล** ที่ประตูถ้ำบอส ⇒ ตรรกะ "ยืนรอแล้วพุ่งเข้าทันทีที่เปิด"
      //     (v6.319/6.325/6.334 ทั้งชุด) **ไม่เคยถูกรันเลยสักครั้ง** — ไหลไปสาขา "ยังเดินไปหาประตู"
      //     ซึ่งสั่ง A* ไป "จุด approach ห่างประตู 140px" = เดินหนีประตูแล้วยืนเฉยจนหมดเวลา
      //   หลักฐานสด รอบ 22:30 (2/8): เข้าโซนตั้งแต่ 22:27:56 · ประตูเปิด 22:30:00 · ยืนนิ่งใน village
      //     จนครบเพดาน 10 นาที (22:37:56) โดยไม่มี log สักบรรทัด — เพราะทุกสาขาที่ log อยู่ใน `_atGate`
      //   ✅ แก้: ทดสอบแบบ "อยู่ในสี่เหลี่ยม + เผื่อขอบ 60px" — ถูกต้องกับโซนทุกขนาด ไม่ต้องจูนเลขอีก
      const _atGate = !!(_p && _gcx != null
        && Math.abs(_p.x - _gcx) <= (exitZone.width || 0) / 2 + 60
        && Math.abs(_p.y - _gcy) <= (exitZone.height || 0) / 2 + 60);
      if (_atGate) {
        ...
        const gs = bossGateState();
        if (gs.locked) sawLock = true;                      // ป้ายใช้ได้บนแมพนี้ — ต่อไปป้ายหาย = ประตูเพิ่งเปิดจริง
        ...
        // (ข) มีป้าย "ยังไม่เปิด" แต่อ่านตัวนับไม่ได้ → ยังเชื่อป้าย แต่แตะเบาๆ ทุก 45 วิ (กันป้ายค้างแล้วรอเก้อทั้งรอบ)
        // (ค) ป้ายหายหลังเคยเห็น = เพิ่งเปิด → พุ่งเข้าทันที · (ง) อ่านอะไรไม่ได้เลย (เกมเปลี่ยนคำ/i18n) → สำรอง 5-8 วิ สุ่ม
        // 🔴 v6.382 — **ผู้ใช้เห็นสด: "เดินชนกำแพงรัว ๆ" จนคนอื่น/แอดมินสงสัย**
        //   ต้นตอคือผลข้างเคียงของ v6.379: `_atGate` เคยเป็นเท็จตลอด สาขานี้จึง **ไม่เคยถูกรัน**
        //   พอแก้ให้ถูกต้อง โค้ด `sawLock ? 0` ก็เริ่มทำงานจริง = ก้าวถอย→ก้าวเข้า **ทุกรอบลูป (~2 วิ) ไม่หยุด**
        //   ⇒ ตัวเลข 0 นี้เขียนไว้ตั้งแต่ v6.334 แต่ไม่มีใครเห็นผลของมัน เพราะสาขาตายมาตลอด
        //   ⚠️ บทเรียน: การ "ปลดล็อกโค้ดที่ไม่เคยรัน" = การปล่อยโค้ดใหม่ที่ไม่เคยผ่านสายตาจริง
        //   ✅ เจตนาเดิม (พุ่งเข้าทันทีที่ประตูเปิด) รักษาไว้ที่ **ครั้งแรกครั้งเดียว**
        //      ครั้งต่อไปเว้น 3.5 วิ (ตามที่คอมเมนต์ v6.325 ตั้งใจไว้แต่แรก) · เกิน 6 ครั้งยังไม่เข้า = เว้น 20 วิ
        //      (6 ครั้งแรกกินเวลา ~20 วิ ซึ่งครอบ "ประตูเพิ่งเปิด" ได้สบาย — ถ้ายังไม่เข้าแปลว่าติดอย่างอื่น
        //       การดันต่อไม่ช่วย มีแต่ทำให้ดูเป็นบอท)
        const pushGap = gs.locked ? 45000
          : (!gatePushes ? 0 : (gatePushes < 6 ? 3500 : 20000));
        if (gs.locked && now() - lastWaitSay > 30000) { lastWaitSay = now(); logInfo('🚪 ประตูยังไม่เปิด (ป้ายบอก แต่อ่านตัวนับไม่ได้) — ยืนรอ แตะประตูห่างๆ ทุก 45 วิ'); }
        if (now() - lastNav > pushGap) {
          lastNav = now();
          if (!gs.locked) gatePushes++;                      // v6.382: นับเฉพาะตอนประตูเปิด (ตอนล็อกเป็นการ "แตะเช็ค" ห่าง 45 วิอยู่แล้ว)
          if (gatePushes === 6) logInfo('🚪 ดันประตู 6 ครั้งแล้วยังไม่เข้า — ชะลอเป็นทุก 20 วิ (ดันถี่ต่อไม่ช่วย + ดูเป็นบอท)');
          // 🔴 v6.325 — **regression ของ v6.319: ยืนทับกลางโซนประตูแล้ว "แตะประตู" ไม่มีผลเลย**
          //   `bossWalkTo` คืน 'arrived' ทันทีถ้าอยู่ในระยะ thresh อยู่แล้ว = **ไม่มีการเดิน** →
          //   พอประตูเปิดจริงก็ไม่มี "การข้ามเส้นประตู" ให้เกมย้ายแมพ → ยืนอยู่นอกถ้ำจนหมดเวลา
          //   หลักฐานสด: รอบ 13:30 ยืนหน้าประตู 13:27→13:38 (11 นาที ประตูเปิดตั้งแต่ 13:30) แล้วยอมแพ้
          //   = **เสียรอบบอสทั้งรอบ** (รอบ 10:30 ที่ยังเป็น v6.318 เข้าได้ปกติ)
          //   แก้: ทำ "ก้าวถอย → ก้าวเข้า" จริง ๆ ทุก 3.5 วิ — ได้การเคลื่อนที่ข้ามเส้นจริง
          //        แต่ยังห่างพอไม่ให้ดูเป็นบอทกระตุก (เดิม 1.2 วิ + สลับวิธีถี่ = อาการที่ผู้ใช้เห็น)
          const bx = 700 - _gcx, by = 500 - _gcy, bd = Math.hypot(bx, by) || 1;   // ทิศเข้าหากลางแมพ
          await bossWalkTo(_gcx + bx / bd * 70, _gcy + by / bd * 70, { thresh: 14, maxMs: 700 });   // ถอยออก ~70px
          await bossWalkTo(_gcx, _gcy, { thresh: 8, maxMs: 1100 });                                  // แล้วเดินเข้าประตูจริง
          if (bossMapId() !== cur) return true;             // ข้ามประตูสำเร็จ
        }
        await sleep(500);
        continue;
      }
      // 🚶 v6.334: ยังไม่ถึงปากประตู (เดินเข้าไปหาอยู่) — ถ้าเกมบอกว่าประตูยังล็อก **ไม่ต้องรีบ**
      ...
      if (!_lockedNow                                   // ประตูล็อก = การไม่ขยับเป็นเรื่องปกติ ไม่ใช่ stuck (v6.334)
        ...
        // 🔴 v6.379: เดิมเขียน `now() - lastNav >= STUCK_JUDGE_MS` (1,600ms) — **เป็นไปไม่ได้ทางคณิตศาสตร์**
        //   ประตูเปิดแล้ว → คำสั่งเดินถูกยิงทุก 1,200ms · ลูปวนรอบละ ~450-500ms (มี DOM scan)
        //   ⇒ ค่าที่วัดได้ตอนตรวจสูงสุด ≈ 1,200-1,500ms = **ไม่เคยถึง 1,600** ⇒ `useWalk` ไม่มีวันเป็น true
        //   ส่วนตอนประตูล็อก (ยิงทุก 3,500ms) ผ่านข้อนี้ได้ แต่ติด `!_lockedNow` ข้างบนแทน
        //   ⇒ สองเงื่อนไข **ตัดกันเอง** = ทางสำรอง WASD ตายสนิทตั้งแต่ v6.365 (ไม่มี log ให้เห็นด้วย)
        //   เจตนาเดิมคือ "คำสั่งได้ถูกส่งไปแล้ว" เท่านั้น — 600ms พอ (ตัวตัดสินจริงคือ _stuckMs)
        && lastNav && now() - lastNav >= 600            // คำสั่งล่าสุดถูกส่งไปแล้วจริง
      ...
      // 👁️ v6.379: สาขานี้ (ยังไม่ถึงโซนประตู) **ไม่เคย log อะไรเลย** ⇒ ตอนบอทค้าง 10 นาทีจึงเงียบสนิท
      //   หาสาเหตุย้อนหลังไม่ได้เลยแม้มี log 1,500 บรรทัด · ทุกนาทีต้องบอกได้ว่า "อยู่ตรงไหน โซนอยู่ไหน"
      if (now() - lastFarSay > 60000) {
        lastFarSay = now();
        logInfo(`🚪 ยังไม่อยู่ในโซนประตู — ตัวอยู่ (${_p ? `${Math.round(_p.x)},${Math.round(_p.y)}` : '?'})`
          + ` · โซน ${exitZone && exitZone.x != null ? `x${exitZone.x} y${exitZone.y} ${exitZone.width || 0}×${exitZone.height || 0}` : '(ไม่รู้พิกัด)'}`
          + ` · ประตู${_lockedNow ? 'ล็อก' : 'เปิดแล้ว'} · วิธี=${useWalk || !_lockedNow ? 'WASD' : 'A*'}`);
      }
      // 🧍 v6.382: สลับครบแล้วยังนิ่ง = ทางนี้ไปไม่ได้จริง (ติดกำแพง) → ชะลอคำสั่งเดินเป็น 4 วิ
      //   ดันทุก 1.2 วิ ทั้งที่ตัวไม่ขยับ = "เดินชนกำแพงรัว ๆ" ที่คนอื่นเห็น — ไม่ได้ช่วยให้เข้าเร็วขึ้นเลย
      const _navGap = _lockedNow ? 3500
        : (switchN >= SWITCH_MAX && _stuckMs >= STUCK_JUDGE_MS ? 4000 : 1200);
      if (!walkInFlight && now() - lastNav > _navGap) {
        lastNav = now();
        // 🚪 v6.379: **ประตูเปิดแล้วต้องเดินเข้าโซนจริง** — จุด approach (ห่าง 140px) มีไว้ตอนประตูล็อกเท่านั้น
        //   พอเปิดแล้วยังวิ่งไปจุดนั้น = เดินหนีประตูทุก 1.2 วิ จนหมดเวลา (อาการจริงรอบ 22:30)
        if ((useWalk || !_lockedNow) && exitZone && exitZone.x != null) {
  ...
  async function bossFight(maxMin) {
    ...
    // 🍲 v6.310 (ผู้ใช้สั่ง): กิน "ต้มปลาร้อน" (+20% เลือดสูงสุด) ตรงนี้ — **หลังเจอบอสในถ้ำแล้ว** (ย้ายมาจาก ensureBossBaitStock ที่กินก่อนเข้าถ้ำ = เปลืองถ้าบอสไม่มา)
    //   ซื้อไว้แล้วตอน stockBossStew (นอกถ้ำ ต้องใช้ร้าน) · กินในถ้ำได้ (useConsumable = กระเป๋า เหมือนยาฟื้นเลือด) · บัฟ 20 นาที ครอบทั้งไฟต์
    if (isOn('bossStew')) { busy = true; try { await drinkBossStew(); } catch (e) { logErr('กินต้มปลาร้อนล้มเหลว', e); } finally { busy = false; } }
    ...
    // 👹 v6.275: บอกผู้เรียกว่า "ฆ่าได้ไหม" — ตัวตัดสินว่าจะกลับเข้าถ้ำไปตีต่อ หรือจบรอบกลับไปฟาร์ม
    //   บอสหาย/หมดเวลาก็ถือว่าไม่ต้องกลับเข้าไปอีก (ไม่มีอะไรให้ตีแล้ว) — นับเฉพาะ "เราออกมาเองทั้งที่บอสยังอยู่"
    //   `!bossSeen` = ไม่เจอบอสเลย (noshow) → กลับเข้าไปอีกก็ไม่มีอะไรให้ตี ถือว่าจบเหมือนกัน
    //   เหลือกรณีเดียวที่ต้องกลับเข้าไป: **เห็นบอสแล้วแต่ไม่ได้ฆ่า** (ตาย/เด้งออก) ← เคสในวิดีโอ
    //   ถ้าบอสหายไปจริง (คนอื่นฆ่า) รอบถัดไปจะได้ noshow แล้วลูปหยุดเอง = แก้ตัวเองได้ ไม่ต้องเดา
    bossLastKilled = killed || !bossSeen;
  ...
  async function ensureBossBaitStock() {
    ...
    try {
      ...
      await stockBossStew();   // ซื้อไว้เฉย ๆ (ต้องใช้ร้าน = นอกถ้ำ) · 🍲 v6.310: **ย้ายการ "กิน" ไปทำในถ้ำหลังเจอบอส** (กันเปลืองถ้าบอสไม่มา)
    ...
    try {
      ...
      if (!resumeHome) {
        ...
        if (!reached) { say('👹 ไปถ้ำบอสไม่สำเร็จ — กลับบ้าน'); bossEvent('❌ เดินไปถ้ำไม่สำเร็จ — กลับบ้าน'); }
          ...
          // 👹 v6.275 — **ออกแบบใหม่: ล่าบอสเป็น "ต่อรอบ" ไม่ใช่ "ต่อทริป"**
          //   ผู้ใช้ส่งวิดีโอ 16:39 มาให้ดู: ตาย → เด้งออกมาที่บ่อตกปลา → **บอทกลับไปตกปลาต่อ**
          //   ทั้งที่ในเฟรมเดียวกันบอสยังมีเลือด 227,970/510,000 และป้าย "ถึงรอบบอสแล้ว!" ยังค้างอยู่
          //   ต้นตอเชิงออกแบบ (ไม่ใช่บั๊กบรรทัดเดียว): `bossFight` จบเมื่อไร = จบทริป = กลับบ้าน + คูลดาวน์ 10 นาที + disarm
          //     → ล้มเหลวครั้งเดียวไม่ว่าด้วยเหตุใด = **ทิ้งรอบบอสทั้งรอบ** ทั้งที่รอบเปิดยาว (raidWindowMinutes 60)
          //   ที่ถูกคือ: กลับเข้าไปใหม่เรื่อยๆ จนกว่า "บอสตาย / รอบปิด / หมดงบเวลา" อย่างใดอย่างหนึ่ง
          //   ✅ เฟรมวิดีโอยังยืนยันว่าง่ายกว่าที่คิด — ป้าย "ประตูบอส" อยู่ที่ **บ่อตกปลาเอง**
          //     คือแมพเดียวกับที่ตายแล้วถูกเด้งมา → กลับเข้าถ้ำเป็นการเดินสั้นๆ ไม่ใช่การข้ามแมพ
          const huntT0 = now();
          for (let roundTry = 1; roundTry <= BOSS_REENTRY_MAX; roundTry++) {
            ...
            if (!await bossTravelTo(bossActiveCave())) {
              say(`👹 เดินกลับเข้าถ้ำไม่สำเร็จ — ${bossNavFail || 'ไม่ทราบสาเหตุ'} · เลิกรอบนี้`);
              bossEvent(`❌ กลับเข้าถ้ำไม่ได้ — ${bossNavFail || '?'}`);
              break;
            }
          }
  ...
  // 👹 v6.275: ล่าบอสเป็น "ต่อรอบ" — ตาย/เด้งออกแล้วบอสยังอยู่ ต้องกลับเข้าไปใหม่ ไม่ใช่กลับบ้านตกปลา
  //   งบเวลา 12 นาที: รอบบอสจริงเปิด ~60 นาที แต่บอสมักตายใน 3-6 นาที · 12 นาทีจึงคลุมกรณีบอสอึดโดยไม่ทิ้งการฟาร์มนาน
  ...
  //   ❌ v6.297 ยังเป็น "กดแล้วหยุดตกปลาทันทีถาวร" = ผิด (ผู้ใช้กดแล้วบอทไม่ตกทั้งวัน — เจอสด 2 ครั้ง)
  //   ✅ ตรงตามป้ายปุ่ม ("อัตโนมัติก่อน N นาที") + ที่ผู้ใช้สั่ง: เปิดปุ่มนี้ = ตกปลาไปเรื่อยๆ → พอบอสใกล้ (≤ lead) →
  //      หยุดตก + ออกไปล่า → ล่าเสร็จ/บอสตาย → min กลับ > lead → กลับมาตกปลาเอง
  //   กันป้าย "ถึงรอบบอสแล้ว" ค้าง: bossTimerMin มีตัวทำนาย v6.211 → คืนเวลาจริง (ไม่ใช่ 0) → ไม่หยุดตกก่อนเวลา
  ...
  function bossHuntDue() {
    ...
    // ✂️ v6.368 — **คูลดาวน์ห้ามกินข้ามไปบล็อกรอบถัดไป** (เคสจริง 2/8/69 ที่ผู้ใช้ต้องกดล่าด่วนเอง)
    //   log ของบอทเองฟ้องตรง ๆ:
    //     23:59 ผู้ใช้กด "ล่าบอสด่วน" (ก่อนบอส 11 นาที) → เทสต์เดินตัวละครไม่ผ่าน (แท็บไม่โฟกัส)
    //     00:07:32 ⏳ ถึงเวลาล่าแล้ว (บอสอีก 3 นาที ≤ ตั้งไว้ 3) แต่ยังไม่ไป — **คูลดาวน์เหลืออีก 187 วิ**
    //   187 วิ ⇒ หมดตอน 00:10:39 · บอสมา 00:10:59 · เดินไปถ้ำ ~26 วิ ⇒ **ไม่มีทางทัน = เสียรอบ**
    //   ⚠️ ทริปนั้น**ล้มตั้งแต่ยังไม่ออกจากบ้าน** (ไม่เคยถึงถ้ำ ไม่เคยเห็นบอส) แต่กลับได้คูลดาวน์เท่าล่าสำเร็จ
    //   หลักการ: คูลดาวน์มีไว้กัน "วิ่งวนรัว ๆ" ไม่ใช่มีไว้ทำให้พลาดรอบที่กำลังรออยู่
    //   ⇒ พอถึงเวลาที่ควรออกเดินทางของรอบถัดไปแล้ว คูลดาวน์หมดสิทธิ์ขวาง
    //   กันวนรัว: ยังคงเว้นอย่างน้อย 45 วิ นับจากครั้งก่อนเสมอ (เพดานล่างเดิม) + bossCtrlFails ≥ 3 ยัง disarm ทั้งรอบเหมือนเดิม
    let coolLeft = coolMs - (now() - lastBossHuntAt);
    if (coolLeft > 0 && now() - lastBossHuntAt >= 45000) {
      try {
        const nx = bossNextSpawnMs();
        const goAt = nx ? nx.at - clamp(cfg.bossLeadMin, 1, 60) * 60000 : 0;
        if (goAt && Date.now() >= goAt) coolLeft = 0;
      } catch {}
    }
  ...
  // ⚖️ v6.348 — **ระบบ "น้ำหนักกระเป๋า" ของเกม (บอทไม่เคยรู้จักมาก่อน = ต้นเหตุ "เปิดบอทแล้วไม่ตกปลา")**
  //   ค่าจริงจาก /api/config: `bagKgPerSlot 7` · `bagKgPerLevel 2` · `bagHeavyStart 0.8` · `bagSlowPct 0.4` · `bagDrainPct 0.6`
  //   ข้อความในเกม (i18n) บอกกติกาไว้ตรงๆ: **"เต็มอย่างใดอย่างหนึ่งก็เหวี่ยงไม่ได้"** (ช่อง หรือ น้ำหนัก)
  //     `hud.msg.bagTooHeavy` = "กระเป๋าหนักเกินพิกัด! ขายปลาหรือขยายช่องก่อนนะ"   ← ตัวบล็อกการเหวี่ยง
  //     `hud.msg.bagHeavy`    = "กระเป๋าหนักมาก — เดินช้าลง พลังงานหมดไวขึ้น! รีบขายปลาซะ"  ← เตือนที่ 80%
  //     `hud.bag.tip`         = "กระเป๋า — ช่องว่าง {free}/{slots} ช่อง · น้ำหนัก {kg}/{cap} กก."
  //   เดิมบอทจับแต่คำว่า "กระเป๋าเต็ม" → ข้อความน้ำหนัก **ไม่เข้าเงื่อนไขไหนเลย** → ไม่ขาย ไม่ระบาย ไม่หยุด = ยืนเงียบ
  //   ⚠️ ที่ 80% ไม่ใช่แค่ "เตือน" — เกมหักจริง: เดินช้าลง 40% + พลังงานหมดไวขึ้น 60% ⇒ ขายก่อนถึงเพดานคือกำไร
  ...
  // "หนักจนเหวี่ยงไม่ได้" — เชื่อข้อความของเกมก่อน (ground truth) · ตัวเลขเป็นตัวเสริมตอนอ่านได้
  const bagTooHeavy = () => {
    try {
      const w = warnText();
      if (w && BAG_HEAVY_BLOCK_RE.test(w)) return true;
      return !!gameTextMatch(BAG_HEAVY_BLOCK_RE);
    } catch { return false; }
  };
  ...
  function popupWatchdog() {
    ...
    const w = warnText();
    // ⚖️ v6.348: เพิ่มข้อความน้ำหนักกระเป๋า — ไม่ใช่ป๊อบอัพค้าง แต่เป็นเหตุที่ระบบขาย/ระบายรับผิดชอบอยู่
    if (w && /กระเป๋าเต็ม|หนักเกินพิกัด|กระเป๋าหนักมาก|ไม่มีเหยื่อ|เหยื่อหมด|พลังหมด/.test(w)) { uiBlockedSince = 0; return false; }
  ...
  function yadReadPanel() {
    try {
      ...
      // 🐛 v6.289: รุ่น v6.288 ใส่ "คำที่เดาว่าแผงต้องมี" (ต้องการ/สะสม/แลกเลย) — **ไม่มีหลักฐานว่าแผงจริงมีคำพวกนั้น**
      //   ผลจริง: เปิดแผงแล้วจับไม่ได้เลยสักครั้ง (`tokpla_yad_raw: []`) = เปลี่ยนการเดาแบบหนึ่งเป็นอีกแบบ
      //   ⇒ เลิกเดาคำ ใช้ **สัญญาณเชิงโครงสร้าง** ที่วัดได้จากข้อมูลจริงแทน:
      //     ปุ่มพร้อมท์ที่หลอกรุ่นแรก มีข้อความ `🎣 แลกปลา (E) Lv.8 100 ขั้น9` = **25 ตัวอักษร**
      //     ส่วนแผงจริงตอน 19:04:17 มีรายการของหลายบรรทัด = ยาวกว่ามาก
      //   → เกณฑ์: มีคำว่า "แลกปลา"/"หยัด" · ไม่อยู่ใน <button> · **ยาว ≥ 60** (กันปุ่ม 25 ตัวอักษร) และ < 900
      // ✅ v6.289 — **อ่านโครงจริงจากแผงที่เปิดสดแล้ว** (เลิกเดาทั้งคำและรูปประโยค):
      //   🎣 ลุงหยัด — เอาปลามาแลกรางวัล ส่งเข้ากล่องจดหมาย            [✕]
      //      ปลาหยุด ×100 | 0/100 | เหยื่อปลอมปลาเงิน x20 | จะได้      [ปลาไม่พอ]
      //      ปลาหยุด ×150 | 0/150 | ☕ กาแฟนักตกปลา x2      | จะได้      [ปลาไม่พอ]
      //   ⇒ "ของที่ต้องเก็บ" อยู่ในรูป `<ชื่อ> ×<เป้า>` ตามด้วยบรรทัด `<มี>/<เป้า>` เสมอ
      //     ส่วน "ของรางวัล" อยู่หลังจากนั้นและใช้ `x` เล็ก (ไม่ใช่ `×`) — แต่ไม่พึ่งความต่างนั้น
      //     เพราะมี element ย่อยที่เป็น **แถวความต้องการล้วน ๆ** (`ปลาหยุด ×100\n0/100`) ให้จับตรง ๆ
      //   → จับเฉพาะ element ที่ข้อความ **ทั้งก้อน** ตรงรูปนี้เป๊ะ = ไม่มีทางปนของรางวัล
      const rows = [];
      for (const d of document.querySelectorAll('div,span')) {
        if (isBotUI(d) || !d.offsetParent) continue;
        const t = (d.innerText || '').trim();
        if (t.length > 60) continue;
        const m = /^(\S[^\n×]{2,28}?)\s*×\s*(\d{1,5})\s*\n\s*(\d{1,5})\s*\/\s*(\d{1,5})\s*$/.exec(t);
        if (!m) continue;
        const need = +m[2], have = +m[3], tgt = +m[4];
        if (need !== tgt) continue;                       // เป้าสองที่ต้องตรงกัน = ยืนยันว่าอ่านถูกแถว
        rows.push({ name: m[1].trim(), need, have });
      }
      if (!rows.length) return null;
      // รวมเป็น "ชนิด → จำนวนสูงสุดที่ต้องใช้" (ลุงมีหลายรายการที่ใช้ปลาชนิดเดียวกัน เช่น 100 กับ 150)
      //   ต้องเก็บให้ถึงรายการที่มากที่สุด ไม่งั้นแลกรายการใหญ่ไม่ได้
      const map = {};
      for (const r of rows) map[r.name] = Math.max(map[r.name] || 0, r.need);
      const raw = rows.map((r) => `${r.name} ${r.have}/${r.need}`).join(' · ');
    ...
    try {
      ...
      if (cfg.sellJunk && !junkAutoSold()) {
        ...
        if (jt) {
          ...
          // 🔴 v6.318 — **race condition ที่ทำให้ขยะ 319 ชิ้นค้างทั้งวัน (พิสูจน์สดแล้ว)**
          //   ลำดับจริงที่เกิด: ขายปลาเสร็จ → แท็บปลาว่าง → ปุ่ม "ขายทั้งหมด" ถูก **disable**
          //   → กดสลับไปแท็บขยะ → อ่านสถานะปุ่มที่ 350ms ครั้งเดียว → React ยังไม่ทันเปิดปุ่มคืน
          //   → เข้าใจว่า "ไม่มีขยะให้ขาย" ทั้งที่แท็บบอกชัดว่ามี 319 ชิ้น (ทดสอบมือ: กดขายได้ +1,423 🪙 ปกติ)
          //   ซ้ำร้าย เกมเลิกใส่ยอดเงินในปุ่มแล้ว ("ขายทั้งหมด" เฉย ๆ) → readTotalCoins() คืน 0 เสมอ
          ...
          if (jTotal > 0 || (jBtn && !jBtn.disabled)) {
            say(`ขายขยะทั้งหมด (${jTotal > 0 ? jTotal.toLocaleString() + ' 🪙' : `${jn ?? '?'} ชิ้น — ขายเพื่อเปิดช่อง`})...`);
            await sellAllCurrentTab();
            // ตรวจผลจริงหลังขาย (ไม่เชื่อว่า "กดแล้ว = ขายแล้ว") — ยังเหลือ = ฟ้องให้รู้ ไม่เงียบ
            await sleep(600);
            const left = junkCountInBag();
            if (left != null && left > 0) logWarn(`🗑️ ขายขยะแล้วแต่ยังเหลือ ${left} ชิ้น — จะลองใหม่รอบขายถัดไป`);
          } else if (jn != null && jn > 0) {
            // แท็บยืนยันว่ามีขยะ แต่ปุ่มไม่ยอมพร้อมใน 2 วิ = ผิดปกติ ต้องเห็นใน log ไม่ใช่เงียบหาย
            logWarn(`🗑️ มีขยะ ${jn} ชิ้นแต่ปุ่ม "ขายทั้งหมด" ยังไม่พร้อมใน 2 วิ — ข้ามรอบนี้ ลองใหม่รอบหน้า`);
          } else {
            say('ไม่มีขยะให้ขาย');
          }
...
📈 กำไรสุทธิสะสม <b>${signed(lifeNet())}</b> 🪙 (หักต้นทุนครบ)`);
  ...
  function tick() {
    ...
    if (isOn('npcYad') && now() - lastYadChk > 700) {   // v6.289: 2 วิยังพลาด — แผงปิดเร็วกว่านั้น
      lastYadChk = now();
...
` +
        ...
        // ⏱️ v6.362 (แก้ช่องโหว่ที่เจอตอนตรวจสด): `bossTimerDom()` ถูกเรียกเฉพาะตอนเดินทาง/อยู่ในถ้ำเท่านั้น
        //   ⇒ ระหว่างตกปลาปกติไม่มีใครอ่านตัวนับเลย → anchor ว่างตลอด → ตอนถึงเวลาออกเดินทางก็ยังต้องเดาจากตาราง
        //   = ทั้งระบบที่ทำมาไม่ได้ทำงานจริงสักครั้ง · ต้องเก็บ "ตอนที่ตัวนับอยู่บนจอ" ซึ่งคือระหว่างฟาร์มนี่แหละ
        //   ทุก 30 วิพอ (ตัวนับเดินวินาทีละ 1 — เก็บบ่อยกว่านี้ไม่ได้อะไรเพิ่ม แต่เปลือง TreeWalker)
        ...
        const warn = warnText();
        // ⚖️ v6.348: "กระเป๋าหนักเกินพิกัด" = เหวี่ยงไม่ได้เหมือนกระเป๋าเต็ม แต่**คนละข้อความ** → ต้องเข้าเส้นทางระบายเดียวกัน
        //   หา 2 ทาง: กล่องเตือนปกติ + สแกนข้อความเกมทั้งหน้า (เผื่อเกมโชว์เป็น toast คนละกล่อง)
        //   พลาดจุดนี้ = บอทยืนเงียบทั้งวันโดยไม่มีระบบไหนรับผิดชอบ (อาการที่ผู้ใช้เจอ)
        const heavyBlock = now() - warnAt > 4000 && ((warn && BAG_HEAVY_BLOCK_RE.test(warn)) || bagTooHeavy());
        if ((warn && now() - warnAt > 4000) || heavyBlock) {
          if (heavyBlock || /กระเป๋าเต็ม/.test(warn || '')) {
            warnAt = now();
            pendingCast = 0; failedCasts = 0;
            // ⚖️ v6.348: แยกให้ออกว่า "ช่องเต็ม" หรือ "น้ำหนักเกิน" — วิธีแก้ต่างกัน (ขยายช่อง = ได้ทั้งช่องและ กก.)
            if (heavyBlock && !bagHeavySaid) {
              bagHeavySaid = true;
              const bw = bagWeight();
              logWarn(`⚖️ กระเป๋า**หนักเกินพิกัด** — เกมห้ามเหวี่ยง${bw ? ` (${bw.kg}/${bw.cap} กก. = ${bw.pct}%)` : ''} · กำลังระบายปลาให้`);
              if (isOn('tgOn') && isOn('tgWarn')) void tgSend(`⚖️ <b>กระเป๋าหนักเกินพิกัด</b> — เกมห้ามเหวี่ยง${bw ? ` (${bw.kg}/${bw.cap} กก.)` : ''}\nบอทกำลังขาย/ระบายให้เอง · ถ้าเจอบ่อย = ขยายช่องกระเป๋า (ช่องละ +7 กก.)`);
            }
            if (!isOn('sell')) {
              stopBot(heavyBlock ? 'กระเป๋าหนักเกินพิกัด ⚖️ — เปิดระบบขายอัตโนมัติ หรือขายปลาเอง (ขยายช่องก็เพิ่มเพดาน กก. ได้)' : 'กระเป๋าเต็ม 🎒 — เปิดระบบขายอัตโนมัติ หรือขายเอง');
          ...
          // ⚖️ v6.348: เกมเตือน "กระเป๋าหนักมาก — เดินช้าลง พลังงานหมดไวขึ้น" ที่ 80% ของเพดาน (bagHeavyStart)
          //   นี่ไม่ใช่แค่คำเตือน — เกมหักจริง: เดินช้าลง 40% (bagSlowPct) + พลังหมดไวขึ้น 60% (bagDrainPct)
          //   ⇒ ขายทันทีคือกำไร (ยังเหวี่ยงได้อยู่ ไม่ต้องรอให้ตัน) · throttle 3 นาที กันเปิดกระเป๋าถี่
          if (warn && BAG_HEAVY_WARN_RE.test(warn) && isOn('sell') && !busy && !orchestrating && now() - bagHeavySellAt > 180000) {
            bagHeavySellAt = now(); warnAt = now();
            const bw = bagWeight();
            say(`⚖️ กระเป๋าหนักมาก${bw ? ` (${bw.kg}/${bw.cap} กก. = ${bw.pct}%)` : ''} — เกมหักความเร็ว 40% + พลังหมดไวขึ้น 60% → ขายก่อนเลย`);
            lastCheck = casts;
            void runSell(true);
            return requestAnimationFrame(tick);
          }
  ...
  function buildPanel() {
    ...
    //   กดแล้ว: เปิดบอท + เปิดโหมดล่าบอส + ตั้ง bossOnly → tick จะวิ่งเฉพาะเส้นทางบอส ไม่แตะการตกปลาเลย
    {
      ...
      const paint = () => {
        ...
        soloBtn.textContent = isOn('bossOnly')
          ? (bossSoloAutoOn
              ...
              : `🎯 ล่าบอสอย่างเดียว: เปิด · ตกปลาอยู่ จะหยุดตกก่อนบอส ${lead} นาที (กดปิด)`)
          : `🎯 "ล่าบอสอย่างเดียว" · หยุดตกเฉพาะช่วงใกล้เวลาบอส (ก่อน ${lead} นาที) แล้วกลับมาตกเอง`;
        soloBtn.style.background = isOn('bossOnly') ? (bossSoloAutoOn ? '#7a3f9e' : '#5b3a7a') : '#2d3748';
      };
      soloBtn.addEventListener('click', () => {
        if (isOn('bossOnly')) {
          ...
          if (!enabled) toggle();
          // 🎯 v6.299: ไม่หยุดตกปลาทันทีแล้ว — ตกปลาไปก่อน จะหยุดตกเฉพาะช่วงใกล้เวลาบอสเพื่อออกไปล่า แล้วกลับมาตกเอง
          say('🎯 โหมดล่าบอสอย่างเดียว: เปิด — ตกปลาไปตามปกติ · พอใกล้เวลาบอสจะหยุดตกแล้วออกไปล่าอย่างเดียว เสร็จแล้วกลับมาตกต่อ');
          if (isOn('tgOn')) void tgSend('🎯 <b>โหมดล่าบอสอย่างเดียว</b> — ตกปลาปกติ · ใกล้เวลาบอสค่อยหยุดตกไปล่า แล้วกลับมาตกต่อ');
        }
    ...
    // 🎯 v6.299: ตั้งเวลาของ "ปุ่มม่วง (ล่าบอสอย่างเดียว)" — เดิมเป็น checkbox แยก (bossOnlyAuto) ที่ตอนนี้รวมเข้าปุ่มม่วงแล้ว
    panel.appendChild(row(
      '🎯 ตั้งเวลาปุ่มม่วง "ล่าบอสอย่างเดียว"',
      'เมื่อ**เปิดปุ่มม่วง**ไว้: บอทตกปลาตามปกติ → พอเหลืออีก N นาทีจะถึงเวลาบอส จะ**หยุดตกปลา** (ไม่ตก/ขาย/เควส/NPC) แล้วออกไปล่าบอส → ล่าเสร็จ/บอสตาย → **กลับมาตกปลาเองอัตโนมัติ** · '
      + 'ทำไมหยุดก่อนถึงเวลา: งานที่ค้างตอนถึงเวลา (กำลังดึงปลา/เปิดกระเป๋าขาย/ทริป NPC) คือสาเหตุที่ทำให้ออกช้า ไปถึงถ้ำไม่ทัน · '
      + 'ค่านี้ควร **≥ "ไปก่อน (นาที)"** (ระบบใช้ค่ามากกว่าอยู่แล้ว) · ต่างจากปุ่มแดงตรงที่ปุ่มม่วงจะตัดงานอื่นทิ้งเพื่อออกให้ทันแน่ ๆ',
    ...
    sectionHead('🏪 NPC (ลุงคลัง / ยายแก่น / ลุงหยัด)', false);
  ...
  function fishStallWatch() {
    try {
      ...
      const why = paused ? 'พักชั่วคราวอยู่ (กด Alt+P หรือ /resume เพื่อเล่นต่อ)'
        ...
        : S(bagTooHeavy, false) === true ? 'กระเป๋าหนักเกินพิกัด (เกมห้ามเหวี่ยง) — ต้องขาย/ระบายปลาก่อน'
        : orb !== 'พร้อม' ? `ปุ่มตกปลา${orb} (เหยื่อหมด/กระเป๋าเต็ม/น้ำหนักเกิน/ยืนนอกบ่อ?)`
      ...
      const detail = `เกม=${S(gameState)} · แมพ=${S(bossMapId)}${inCave ? '(ถ้ำบอส)' : ''} · เฟสบอส=${bossPhase}`
        ...
        + ` · น้ำหนักเป๋า=${S(() => { const b = bagWeight(); return b ? `${b.kg}/${b.cap} กก.(${b.pct}%)` : 'อ่านไม่ได้'; })} · กล่องเตือน="${S(warnText, '-')}"`
