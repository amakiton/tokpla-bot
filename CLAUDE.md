...
|---|---|
| `docs/HANDOFF.md` | **อ่านก่อนเสมอเมื่อเริ่มแชทใหม่** — สถานะล่าสุด · งานค้าง · กฎที่ต้องรู้ก่อนแตะอะไร |
...
|---|---|---|
| สถิติ/log/cfg/สถานะบอท | เปิดแท็บไป `https://game.fishbonecast.com/api/config` (origin เดียวกับเกม แต่เป็นแค่ JSON) แล้วอ่าน `localStorage` คีย์ `tokpla_*` | **ไม่** — แต่ต้อง v6.397+ ⚠️ ดูด้านล่าง |
...
🔴 **บรรทัด "ปลอดภัยเสมอ" ที่เคยเขียนไว้ตรงนี้ผิด และทำให้บอทตายเงียบ 106 นาที (5 ส.ค.)**
`@match *://*.fishbonecast.com/*` กิน **ทุกพาธ** รวม `/api/config` ⇒ แท็บที่เปิดไป "อ่านเฉย ๆ"
ก็โหลดบอทอีกตัวเต็ม ๆ · ตอนปิดแท็บ `pagehide` เรียก `persistEnabled()` ด้วย `enabled=false`
⇒ **ลบธง `tokpla_bot_resume` + เขียน `tokpla_bot_enabled='0'`** ⇒ แท็บบอทตัวจริงโหลดใหม่แล้วไม่เปิดต่อ
**v6.397 อุดแล้ว** (สคริปต์ `return` ทันทีบน `/api/*` + `persistEnabled` ไม่ให้หน้าที่ไม่เคยเปิดบอทเขียนทับ)
⇒ ก่อนเชื่อว่า "การอ่านไม่กระทบระบบ" ให้ตรวจ `@match` ก่อนเสมอ — เครื่องมือวินิจฉัยที่รันในระบบ
เดียวกับที่ถูกวินิจฉัย **คือส่วนหนึ่งของระบบ**
